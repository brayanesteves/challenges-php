<?php
 class ACL {
    ...
    public function getTypesUsers() {
        //echo "SELECT `UsrTyp_Rfrnc` FROM `0_Usrs` WHERE `Rfrnc` = '{$this->_referenceUser}';";
        $typesUser = $this->_db->query("SELECT `UsrTyp_Rfrnc` FROM `0_Usrs` WHERE `Rfrnc` = '{$this->_referenceUser}';");
        $typesUser = $typesUser->fetch();
        //echo $typesUser['UsrTyp_Rfrnc']; exit;
        if(!is_bool($typesUser) || $typesUser == false) {
            $typesUser['UsrTyp_Rfrnc'] = 0;
        } else {
            $typesUser = $typesUser;
        }
